...
- 설계 방향) 아이디/패스워드/대화명/성명 + 입력 후 등록 누르면 -> 해당 디비에 저장하여 로그인 버튼 눌렀을 때 그 값을 꺼내와서 "로그인이 성공되었습니다." 라고 구현예정

  
- 회원가입 창에서 MemberShipView을 이용하여 값을 받아옴 -> 처음엔 무작정 클래스 생성자 안 this를 통해서 값을 getText() 하려고 했는데, 처음 설계 방향 자체를 잘못 잡았다. -> 그래서 테이블 스키마 자체를 다시 설계하여 디비와 연동했다. 그리고 해당 쿼리문을 다시 짜서 -> select 문으로 nickname을 찾고 -> setString 자체를 username/password로 진행했다. (findNickName 메소드 구현) 해서 회원가입 창에서 값을 읽어들이는 부분을 해결했다.
  
- 해당 부분을 진행하면서 디비의 중요성과 편리함을 알게 되었다. 그리고 처음엔 단순히 인스턴스화를 통한 전역변수 이용을 하여 구현 하려고 했다. 처음엔 DB를 설계를 하지 않았는데, 아이디,패스워드, 회원정보를 저장 해야 된다는 필요성을 느껴 toad에서 테이블 등록(member) 후 아이디와 패스워드, 닉네임(회원가입창)에서 받아와서 Dao 클래스에 (Save) 메소드로 구현했다. 
...
- 1번 로그인 구현과 관련되서 두 클래스는 유기적으로 동작 한다는 점을 다시 한 번 느꼈다.



![image](https://github.com/Ohsaam/ChatJava/assets/96507825/3f9750aa-40a3-4690-ab0a-df5b91c740b8)

등록을 눌렀을 때 -> 해당 정보가 디비에 저장 되어야 한다 -> MemberListFrame에 MemberShipView의 정보를 받아와 저장해야 된다. --> insert문 수정 --> 날짜 읽어오는 부분 삭제 --> 이 부분 때문에 매핑이 안 됐음 (수정완료)

- 처음에 DB를 설계를 하지 않았는데, 아이디,패스워드, 회원정보를 저장 해야 된다는 필요성을 느껴 toad에서 스키마 설계를 하고 테이블 등록(member) 후 아이디와 패스워드, 닉네임(회원가입창)에서 받아와서 Dao 클래스에 (Save) 메소드로 구현했다.
